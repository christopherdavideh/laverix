                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('users.update', $user) }}" accept-charset="UTF-8">
                        ...
                        <div>
                            ...
                            <input id="names" class="form-control{{ $errors->has('names') ? ' is-invalid' : '' }}" name="names" value="{{ old('names', $user->names) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="lastnames" class="form-control{{ $errors->has('lastnames') ? ' is-invalid' : '' }}" name="lastnames" value="{{ old('lastnames', $user->lastnames) }}">
                        </div>

                        <div>
                            <x-label for="phone" :value="__('Phone')" />
                            {!! $errors->first('phone', '<span class="invalid-feedback" role="alert" style=" color: red">:message</span>') !!}
                            <input id="phone" class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}" name="phone" value="{{ old('phone', $user->phone) }}">
                        </div>

                        <div>
                            <x-label for="address" :value="__('Address')" />
                            {!! $errors->first('address', '<span class="invalid-feedback" role="alert" style=" color: red">:message</span>') !!}
                            <input id="address" class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" name="address" value="{{ old('address', $user->address) }}">
                        </div>

                        <div>
                            <x-label for="birth" :value="__('Birth')" />
                            {!! $errors->first('birth', '<span class="invalid-feedback" role="alert" style=" color: red">:message</span>') !!}
                            <input id="birth" type="date" class="form-control{{ $errors->has('birth') ? ' is-invalid' : '' }}" name="birth" value="{{ old('birth', $user->birth) }}">
                        </div>
